...
app.get("/filter", (req,res)=>{
  ...
  const foundJoke=jokes.findIndex((joke)=> joke.jokeType===type);
  res.status(200).json(foundJoke);
});
//4. POST a new joke
app.post("/jokes", (req, res)=>{
  const joke_type=req.body.joketype;
...
app.delete("/deleteAll", (req,res)=>{
  ...
  if(key===masterKey){
    jokes=[];
    res.status(200).json({"response": "All deleted"});
  }else{
    res.status(200).json({"response": "go away"});
  }
});
